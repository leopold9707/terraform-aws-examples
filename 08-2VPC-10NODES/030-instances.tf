...
resource "aws_eip" "1-admin_fip" {
  vpc        = true
  instance   = aws_instance.instances["1-Primary-01"].id
...
resource "aws_eip" "2-admin_fip" {
  vpc        = true
  instance   = aws_instance.instances["2-Primary-01"].id
